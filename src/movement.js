...
export function setupInputHandling(
  ...
  scene.actionManager.registerAction(
    new BABYLON.ExecuteCodeAction(
      ...
      function (evt) {
        ...
        inputMap[key.match(/[a-zA-Z]/) ? key.toLowerCase() : key] =
  ...
  function pickEnemy(scene, x, y) {
    ...
    var predicate = function (mesh) {
      return (
        mesh.isPickable && mesh.isEnabled() && mesh.name === "enemyClone.Sphere"
      ); // Ensure it's the terrain
    };
...
function handleGamepadInput(gamepad, character) {
  gamepad.onleftstickchanged((values) => {
    ...
    let inputVelocity = new BABYLON.Vector3(values.x, 0, values.y).scaleInPlace(
      0.1,
    );
...
function onKeyDown(event) {
  ...
  // ── Spell hotkeys ──────────────────────────────────────────────────────────
  // 1 — Thunderball: white-blush orb above tab-target → lightning bolt (15 dmg)
  if (event.key === "1") {
...
function rotateToTarget() {
  var forwardTarget = PLAYER.target.position
  ...
  PLAYER.health.rotationCheck.rotationQuaternion =
    BABYLON.Quaternion.RotationYawPitchRoll(forwardAngleTarget, 3.14, 0);
  var rotationQuaternion = BABYLON.Quaternion.RotationYawPitchRoll(
    Math.PI,
    0,
    0,
  );
  PLAYER.health.rotationCheck.rotationQuaternion = rotationQuaternion.multiply(
    PLAYER.health.rotationCheck.rotationQuaternion,
  );
...
function handleCharacterMovement(
  ...
  let moveDirection = new BABYLON.Vector3(
    lastMoveDirection.x,
    lastMoveDirection.y,
    lastMoveDirection.z,
  );
  if (!anim.Roll.isPlaying && !anim.Attack.isPlaying) {
    moveDirection = new BABYLON.Vector3(
      0,
      dummyAggregate.body.getLinearVelocity().y,
      0,
    );
  }
  // let moveDirection = dummyAggregate.body.getLinearVelocity();
  if (inputMap["w"] || inputMap["ArrowUp"]) {
    ...
    if (
      !anim.Roll.isPlaying &&
      !anim.Attack.isPlaying &&
      !anim.Combo.isPlaying
    ) {
      for (let key in anim) {
