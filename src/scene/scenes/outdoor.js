...
export async function createOutdoor(engine) {
  ...
  // ── Advanced TPS Camera (GrudgeCamera) ──────────────────────────────────
  // Falls back to legacy setupCamera if GrudgeCamera fails (e.g. missing lib)
  let camera;
  let grudgeCam = null;
  try {
  ...
  // Patch takeDamage to update the HUD
  const _origTakeDamage = character.health.takeDamage.bind(character.health);
  character.health.takeDamage = function (amount) {
    _origTakeDamage(amount);
