...
export function setupEnemies(scene, player, terrain, amount, mesh, hpbar) {
  ...
  setupTabTargeting(scene, player);
  return enemies;
}

// ─── Tab Targeting ────────────────────────────────────────────────────────────
// Cycles through alive enemies sorted by distance. Tab = next, Shift+Tab = prev.
// Sets player.target and applies a gold outline to the selected enemy.
export function setupTabTargeting(scene, player) {
  let tabIndex = -1;

  window.addEventListener("keydown", (e) => {
    if (e.key !== "Tab") return;
    e.preventDefault(); // block browser focus-switch

    const enemies = scene.meshes.filter(
      (m) => m.name === "enemy" && m.health && m.health.isAlive,
    );
    ...
    enemies.sort(
      (a, b) =>
        BABYLON.Vector3.Distance(a.position, player.position) -
        BABYLON.Vector3.Distance(b.position, player.position),
    );

    tabIndex = e.shiftKey
      ? (tabIndex - 1 + enemies.length) % enemies.length
      : (tabIndex + 1) % enemies.length;

    const selected = enemies[tabIndex];
    ...
    // Gold outline on the tab-selected target
    applyOutlineToMeshAndChildren(
      selected,
      0.025,
      new BABYLON.Color3(1.0, 0.85, 0.1),
    );
...
function addRandomMovement(enemy, scene, terrain) {
  ...
  enemy.interval = setInterval(() => {
    ...
    targetPosition = enemy.position.add(
      new BABYLON.Vector3(randomX, 0, randomZ),
    );

    // Get the terrain height at the new target position
    let terrainHeight = terrain.getHeightAtCoordinates(
      targetPosition.x,
      targetPosition.z,
    );
    ...
    enemy.rotationQuaternion = BABYLON.Quaternion.RotationYawPitchRoll(
      forwardAngleTarget,
      0,
      0,
    );
    ...
    enemy.rotationQuaternion = rotationQuaternion.multiply(
  ...
  scene.onBeforeRenderObservable.add(() => {
    if (enemy.health.isAlive) {
      ...
      enemy.position = BABYLON.Vector3.Lerp(
        enemy.position,
        targetPosition,
        0.003,
      );
      ...
      if (enemy.moveWithCollisions) {
        let moveDirection = targetPosition
  ...
  scene.registerBeforeRender(() => {
    ...
    scene.meshes.forEach((mesh) => {
      if (mesh.name === "enemy" && mesh.health.isAlive) {
        ...
        var forward = scene.activeCamera
          .getFrontPosition(1)
  ...
  scene.registerBeforeRender(() => {
    ...
    scene.meshes.forEach((mesh) => {
      if (mesh.name === "enemy" && mesh.health.isAlive) {
        ...
        // Check if the caster is facing the target
        let dotProduct = BABYLON.Vector3.Dot(
          player.health.rotationCheck.forward,
          directionToTarget,
        );
        // console.log(caster.rotationCheck.forward);
        if (dotProduct < 0.5) {
